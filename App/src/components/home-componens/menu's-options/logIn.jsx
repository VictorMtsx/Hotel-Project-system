...
export function LogIn() {
	...
	async function hundlerSubmitFormLogIn(data) {
		try {
			...
			if (response.ok && responseData.auth && responseData.token) {
				localStorage.setItem("token", responseData.token); // Salva o token no localStorage
				console.log("Token salvo:", responseData.token);
				// Lógica adicional após sucesso no login
			} else {
